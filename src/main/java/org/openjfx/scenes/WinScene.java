...
/**
 * JavaFX WinScene Class that displays T-Bee-D Game Win Screen
 *
 * @version 1.0
 */
...
public class WinScene {
    ...
    public static Scene getWinScreen(PlayerCharacter player) {
        ...
        // Setting up an hbox for weapon images at the center
        HBox hBox = new HBox(20);
        hBox.setAlignment(Pos.CENTER);

        // Creating space between top of stage and win message & adding them to the main vbox
        // Adding final stats to the main vbox
        Region space = new Region();
        vBox.getChildren().add(space);
        vBox.getChildren().add(new GameElements.TitleText("YOU WON!!"));
        vBox.getChildren().add(finalStats);

        // Creating space between win message & play again button
        for (int i = 0; i < 8; i++) {
        ...
        // Adding the hbox containing weapon images to the vbox
